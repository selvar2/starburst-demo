# ============================================================================
# BACKUP: app_jwt.py
# Timestamp:  2026-05-10T06:07:24Z (TS=20260510_113724)
# Source:     gen-ai-project/starburst-mcp2/app_jwt.py
# Branch:     dev6-cp-dev3 @ commit bc67ed4
# Why:        Adding business-user NL DROP TABLE grammar — mirrors the
#             INSERT/UPDATE/DELETE/TRUNCATE patterns. Recognizes 'delete the
#             table', 'remove (the)? table', 'permanently remove (the)? table',
#             '^drop table <FQ>', and '^in catalog/schema X ... drop|delete|
#             remove|permanently-remove the table'.
# Before:     /api/chat handled DROP TABLE only via raw SQL passthrough or the
#             simple 'drop table <tbl>$' regex which auto-qualified bare names
#             but failed on verbose 'drop table X in catalog Y, schema Z' suffix.
#             DELETE trigger's '^in catalog ... delete|remove' branch was too
#             broad — would intercept 'permanently remove the table'.
# After:     New _nl_drop_table_to_sql() parser. Tightened DELETE trigger's
#             '^in catalog/schema ... delete|remove' branches to require
#             'row|record' so they no longer collide with table operations.
#             DROP TABLE trigger placed BEFORE DELETE in the dispatch chain.
# Restore:    cp 'gen-ai-project/starburst-mcp2/backup/app_jwt.20260510_113724.bak.py' gen-ai-project/starburst-mcp2/app_jwt.py
# ============================================================================
